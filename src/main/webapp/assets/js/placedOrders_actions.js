...
jQuery(document)
		.ready(
				function($) {
					...
					var url = "http://localhost:8080/WishlistService/webapi/fullfiller/getOrders/"
							+ fullfiller_id;
					$
							.ajax({
								type : 'POST',
								...
								success : function(items) {
									...
									if (!$.isEmptyObject(items)) {
										...
										for (var i = 0; i < items.length; i++) {
											...
											$("#quantity_" + prod_no).text(
													"Purchased Quantity: "
															+ items[i].quantity
															+ " unit(s)");
											$("#requestedby_" + prod_no)
													.text(
															"Creator's Name: "
																	+ items[i].creator_name);
										...
										var no_items = "<div id='no_items' align='center' style='border: 2px solid #eee;'>"
												+ "<label class='col-form-label'>No orders placed by you for others.</label>"
												+ "</div>";
